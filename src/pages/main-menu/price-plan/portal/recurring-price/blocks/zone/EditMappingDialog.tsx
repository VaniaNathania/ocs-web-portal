import z from "zod";
import { recurringUpdateMappingSchema } from "../../types/form";
...
type RecurringUpdateMappingForm = z.infer<typeof recurringUpdateMappingSchema>;
...
const EditMappingDialog = () => {
  ...
  const methods = useForm<RecurringUpdateMappingForm>({
    resolver: zodResolver(recurringUpdateMappingSchema),
    defaultValues: {
      mappingName: "",
      mappingUnit: [],
    },
  ...
  const DoUpdateMapping = async (data: RecurringUpdateMappingForm) => {
    ...
    try {
      ...
      if (response?.status) {
        await fetchMapping(selectedRatePlan || 0);
  ...
  const onSubmit = async (data: RecurringUpdateMappingForm) => {
