...
import { OfferType, OfferTypeMeta } from "../../blocks/utils/MapDisplayData";
import { OFFER_TYPE_LABEL } from "../../blocks/utils/MapDisplayData";
...
const AllFeatureTabContent: React.FC<AllFeatureTabContentProps> = ({ isOpen, onClose, rowData }) => {
  ...
  const [selectedFeature, setSelectedFeature] = useState<AllFeatureData | null>(null);
  ...
  // Filter states
  const [selectedContactChannel, setSelectedContactChannel] = useState<number[]>([]);
  ...
  const [driverAndLinkage, setDriverAndLinkage] = useState<driverAndLinkageProps[]>([]);
  const [contactChannelData, setContactChannelData] = useState<ContactChannelList[]>([]);
  const [mergedContactChannelData, setMergedContactChannelData] = useState<any[]>([]);

  const [backupFeature, setBackupFeature] = useState<AllFeatureData | null>(null);

  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [currentSorting, setCurrentSorting] = useState<any>([{ id: "attrName", desc: false }]);
  ...
  const renderAttrCatg = (): OfferTypeMeta => {
    const offerType = (rowData?.offerType ?? "2") as OfferType;
    return OFFER_TYPE_LABEL[offerType];
  };
  ...
  const loadDetailData = async (feature: AllFeatureData) => {
    try {
      const response = await GetData(`${API_URL_OFFER}/offer/attr/qry-attr-detail`, {
  ...
  const fetchDriverAndLinkage = useCallback(
    async (attrId: number | null) => {
      ...
      try {
        const response = await GetData(`${API_URL_OFFER}/offer/attr/qry-attr-driver-and-linkage/${attrId}`, {
          attrId,
        });
        if (response.status) {
  ...
  const fetchApplyContactChannel = useCallback(
    async (attrId: number | null, spId: number) => {
      ...
      try {
        const response = await GetData(`${API_URL_OFFER}/offer/attr/qry-attr-apply-channel`, {
          attrId,
          spId,
        });
        if (response.status) {
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchDataType = async () => {
        try {
          const response = await GetData(`${API_URL_OFFER}/offer/common/qry-data-type`, {});
      ...
      const fetchInputTypeList = async () => {
        try {
          const response = await GetData(`${API_URL_OFFER}/offer/common/qry-input-type-list`, {});
          if (response?.data && Array.isArray(response.data)) {
  ...
  useEffect(() => {
    if (isOpen && selectedAttrId) {
      fetchDriverAndLinkage(selectedAttrId);
      fetchApplyContactChannel(selectedAttrId, 0);
    }
  }, [isOpen, selectedAttrId, fetchDriverAndLinkage, fetchApplyContactChannel]);

  useEffect(() => {
    if (contactChannelData.length > 0 && contactChannelList.length > 0) {
      const merged = contactChannelData.map((item) => {
        const matched = contactChannelList.find((channel) => channel.contactChannelId === item.contactChannelId);
        return {
          ...item,
  ...
  const handleAddOnSuccess = async (newAttrId?: number) => {
    try {
      const response = await doGetAllFeatureData(currentPage, pageSize, currentSorting, []);
      ...
      if (newAttrId) {
        const newFeature = response.data.find((f: AllFeatureData) => f.attrId === newAttrId);
  ...
  const handleEditOnSuccess = async () => {
    try {
      ...
      if (selectedFeature?.attrId && response.data) {
        const updatedFeature = response.data.find((f: AllFeatureData) => f.attrId === selectedFeature.attrId);
  ...
  useEffect(() => {
    setRefreshTrigger((prev) => prev + 1);
  }, [debounceSearch, instantiatableValue, projectVisibleValue]);
  ...
  const doGetAllFeatureData = useCallback(
    ...
    [debounceSearch, instantiatableValue, projectVisibleValue, selectedFeature, GetData],
  );
    ...
    () => [
      {
        ...
        id: "attrName",
        header: ({ column }) => <DataGridColumnHeader className="text-gray-800" title="Feature Name" column={column} />,
        enableSorting: true,
        enableHiding: false,
        cell: ({ row }) => {
          const name = row.original.attrName;
          const isSelected = selectedFeature?.attrCode === row.original.attrCode;
          return (
            <div className={`text-gray-800 cursor-pointer p-2 rounded ${isSelected ? "bg-blue-100 font-semibold" : "hover:bg-gray-50"}`} onClick={() => handleFeatureSelect(row.original)}>
              {name}
            </div>
          );
        },
      },
      {
        ...
        id: "attrCode",
        header: ({ column }) => <DataGridColumnHeader className="text-gray-800" title="Feature Code" column={column} />,
        enableSorting: true,
        enableHiding: false,
        cell: ({ row }) => {
  ...
  const renderContent = () => {
    switch (currentMode) {
      ...
      case "add":
        return <FeatureAddForm contactChannelList={contactChannelList} expandedInputTypes={expandedInputTypes} onSubmit={handleAddOnSuccess} onCancel={handleCancel} rowData={rowData} renderAttrCatg={renderAttrCatg} />;
      default:
        return null;
    }
  };
  ...
  return (
    <>
            ...
            <div className="w-1/3 bg-white border border-gray-200 rounded shadow-sm flex flex-col min-h-0">
              ...
              <div className="p-3 border-b">
                <div className="flex justify-between items-center mb-3">
                  <span className="font-semibold text-gray-800">Main Features</span>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                      <button onClick={() => setShowGroupMenu(!showGroupMenu)} className="bg-gray-100 px-2 py-1 rounded text-sm hover:bg-gray-200">
                        ...
                      </button>
                      {showGroupMenu && (
                        <div className="absolute right-0 top-8 bg-white border border-gray-200 rounded shadow-lg p-2 z-10">
                          <button onClick={() => handleShowFeatureGroup(true)}>
                            <span className="text-sm text-gray-700">Feature Group</span>
                          </button>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex-1 overflow-auto px-1">
                <DataGridProvider
                  ...
                  onFetchData={({ pageIndex, pageSize, sorting, columnFilters }) => {
                    ...
                    return doGetAllFeatureData(pageIndex + 1, pageSize, sorting, columnFilters);
                  }}
                />
              </div>
            </div>
      ...
      <FeatureDeleteDialog isOpen={showDeleteDialog} onClose={() => setShowDeleteDialog(false)} onSuccess={handleDelete} selectedFeature={selectedFeature} />

      <FeatureGroupDialog isOpen={showFeatureGroup} onClose={() => setShowFeatureGroup(false)} rowData={rowData} />
    </>
  );
};
