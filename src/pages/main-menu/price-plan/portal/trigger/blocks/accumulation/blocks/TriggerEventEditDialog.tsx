...
import z from "zod";
import { TriggerAcmEventEditSchema } from "../types/forms";
...
type TriggerAcmEventEditFormType = z.infer<typeof TriggerAcmEventEditSchema>;
...
const TriggerEventEditDialog = ({
  ...
  const methods = useForm<TriggerAcmEventEditFormType>({
    resolver: zodResolver(TriggerAcmEventEditSchema),
    defaultValues: {
  ...
  const onSubmit = (data: TriggerAcmEventEditFormType) => {
    doUpdateTriggerAccumulation(data);
  };

  const doUpdateTriggerAccumulation = useCallback(
    async (data: TriggerAcmEventEditFormType) => {
      try {
        setIsLoading(true);
        const response = await PutData(
          `${API_URL}/trigger/event/acm/edit`,
          data
        );
        ...
        if (response?.status) {
          ...
          toast.success("Success Update Accumulation Event Trigger");
          resetForm();
          setShowDialog(false);
        } else {
          toast.error("Error Update Accumulation Event Trigger");
        }
      } catch (error) {
        toast.error("Error Update Accumulation Event Trigger");
      } finally {
  ...
  useEffect(() => {
    ...
    if (showDialog && selectedEvent) {
      setValue("acmThresholdId", selectedThreshold.acmThresholdId);
