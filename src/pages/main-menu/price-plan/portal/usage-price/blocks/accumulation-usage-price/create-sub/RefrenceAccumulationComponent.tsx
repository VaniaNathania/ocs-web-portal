...
import { Plus, Trash2, CheckCircle, Check, Edit } from "lucide-react";
import { apiConfig } from "@/config/api.config";
import { useCallApi } from "@/hooks";
import { Input } from "@/components/ui/input";

export interface ReferenceAccumulation {
...
// Import TimeSpanAccumulation interface from TimeSpanComponent
...
interface ReferenceAccumulationProps {
  data: ReferenceAccumulation[];
  onChange: (data: ReferenceAccumulation[]) => void;
  timeSpanAccumulation: TimeSpanAccumulation[] | null; // Add this prop to get timeSpanAccumulation data
}

const API_URL_PRICE_PLAN = apiConfig.service_price_plan;

const ReferenceAccumulationComponent: React.FC<ReferenceAccumulationProps> = ({
  data,
  onChange,
  timeSpanAccumulation = [], // Default to empty array
}) => {
  ...
  const updateItem = (
    index: number,
    field: keyof ReferenceAccumulation,
    value: any
  ) => {
    const newData = [...data];

    // Handle type conversions and clear errors
  ...
  const addReference = () => {
    const newItem: ReferenceAccumulation = {
      acmTimeSpanId: null, // Default to first index
      effValue: 0,
      expValue: 0,
      resourceId: 0,
      calculationMethod: "F",
    ...
    // Set initial state sebagai unconfirmed
    setItemStates((prev) => ({
      ...prev,
      [data.length]: {
        index: data.length,
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button
          ...
          // disabled={!timeSpanAccumulation || timeSpanAccumulation.length === 0}
        >
          Add
        </Button>
      </div>
        ...
        <div className="space-y-3">
          ...
          {data.map((item, index) => {
            ...
            return (
              ...
              >
                {/* Time Span Reference Selection - shows timeSpanAccumulation items */}
                <div className="col-span-2">
                  <FormControl fullWidth size="small">
                    <Select
                      value={item?.acmTimeSpanId?.toString() || ""}
                      onChange={(event: SelectChangeEvent) =>
                        updateItem(index, "acmTimeSpanId", event.target.value)
                      }
                      ...
                      renderValue={(selected) => {
                        ...
                        if (
                          ...
                          selectedIndex >= 0 &&
                          selectedIndex < timeSpanAccumulation.length
                        ) {
                          const timeSpanItem =
                            timeSpanAccumulation[selectedIndex];
                          return (
                            timeSpanItem.timeSpan || `Index ${selectedIndex}`
                          );
                        }
                    ...
                    >
                      {(timeSpanAccumulation || []).map(
                        (timeSpanItem, tsIndex) => (
                          <MenuItem key={tsIndex} value={tsIndex.toString()}>
                            {timeSpanItem.timeSpan ||
                              `Time Span ${tsIndex + 1}`}
                          </MenuItem>
                        )
                      )}
...
export default ReferenceAccumulationComponent;
