...
const UpdateDialog = () => {
  ...
  const [formField, setFormField] = useState<UpdateAccountItem>({
    acctResId: 0,
    parentId: 0,
  ...
  const doGetListAccountItem = async () => {
    ...
    try {
      ...
      const response = await GetData(
        `${API_URL}/account-item-type/update-list`,
        {
          page: 1,
          size: 1,
          sortBy: "acctItemTypeId",
          sortDirection: "asc",
          acctItemTypeId: selectedAccountItem?.acctItemTypeId,
        },
      );
      ...
      setFormField((prev) => ({
        ...
        exchangeItemTypeId: response?.data[0]?.exchangeItemTypeId,
        acctItemTypeName: response?.data[0]?.acctItemTypeName,
        comments: response?.data[0]?.comments,
        acctItemTypeCode: response?.data[0]?.acctItemTypeCode,
        usageType: response?.data[0]?.usageType,
        spId: response?.data[0]?.spId,
        gstType: response?.data[0]?.gstType,
        feeType: response?.data[0]?.feeType,
        zeroFeePrintFlag: response?.data[0]?.zeroFeePrintFlag,
        defaultTaxItemTypeId: response?.data[0]?.defaultTaxItemTypeId,
        feeClass: response?.data[0]?.feeClass,
        billPriority: response?.data[0]?.billPriority,
        acctItemGroupId: response?.data[0]?.acctItemGroupId,
        billItemType: response?.data[0]?.billItemType,
        taxAcctItemTypeId: response?.data[0]?.taxAcctItemTypeId,
        discountAcctItemTypeId: response?.data[0]?.discountAcctItemTypeId,
        taxApplyId: response?.data[0]?.taxApplyId,
        acctResId: response?.data[0]?.acctResId,
      }));
      ...
      console.error("Error fetching Account Item Type", error);
  ...
  useEffect(() => {
    if (showDialog.show === true && showDialog.mode === "update") {
      setIsFetching(true); // ⏳ langsung tampilkan loader
      doGetListAccountItem().finally(() => {
        setIsFetching(false); // ✅ matikan loader setelah selesai
      });
    }
  }, [showDialog]);
  ...
  const doUpdateAccountItem = async () => {
    try {
      const response = await PutData(
        `${API_URL}/account-item-type/update?acctItemTypeId=${selectedAccountItem?.acctItemTypeId}`,
        formField,
      );
      if (response?.status) {
        handleShowDialog(false, "update", null);
      ...
      //  console.log(error);
      toast.error("Failed to Update Account Item Type");
    }
  };
  ...
  return (
    <Dialog
      open={showDialog.show && showDialog.mode === "update"}
