...
const CalculationObjectConditionForm = () => {
  ...
  const addGroup = () => {
    ...
    setValue("calculationObject.insertDiscountConditionGroup", [
      ...conditions,
      newCondition,
    ]);
  };
  ...
  const removeCondition = (grpId: number, seqNo: number) => {
    ...
    setValue("calculationObject.insertDiscountConditionGroup", newConditions);
  };
        ...
        <div key={grpId} className="p-4 mb-6 border rounded-lg">
          ...
          {groupConditions.map((condition, condIdx) => (
            <div key={condIdx} className="flex items-center gap-2 mb-2">
              <Controller
                control={control}
                name={`calculationObject.insertDiscountConditionGroup.${conditions.findIndex(
                  (c) =>
                ...
                render={({ field }) => {
                  const selectedRefCond = referenceCondition.find(
                    (c) =>
                      c.dpRefCondId ===
                      watch(
                        `calculationObject.insertDiscountConditionGroup.${conditions.findIndex(
                          (c) =>
...
export default CalculationObjectConditionForm;
