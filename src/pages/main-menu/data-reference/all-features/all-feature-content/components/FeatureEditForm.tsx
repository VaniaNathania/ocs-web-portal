...
const FeatureEditForm: React.FC<FeatureEditFormProps> = ({
  contactChannelList,
  attrCatgList,
  detailData,
  expandedInputTypes,
  ...
  const [selectedAttrCatg, setSelectedAttrCatg] = useState<number[]>([]);
  ...
  useEffect(() => {
    if (detailAttrCatg && detailAttrCatg.length > 0) {
      const initialSelected = detailAttrCatg.map((item) => item.attrCatg);
      setSelectedAttrCatg(initialSelected);

      setFormData((prev) => ({
        ...prev,
        attrApplyCatgIds: initialSelected,
        attrApplyCatgRequest: initialSelected.map((catg) => ({
          attrId: detailData.attrId || selectedFeature.attrId,
          attrCatg: catg,
          spId: 0,
        })),
  ...
  useEffect(() => {
    ...
    if (detailData) {
      setFormData((prev) => ({
        ...
        attrApplyCatgIds: detailData.attrApplyCatgIds || prev.attrApplyCatgIds || [],
        attrApplyChannelIds: detailData.attrApplyChannelIds || prev.attrApplyChannelIds || [],
        attrApplyCatgRequest:
          detailData.attrApplyCatgRequest ||
                ...
                <PopoverTrigger asChild>
                  <button type="button" className="w-[170px] px-2 py-1 text-sm h-10 border border-gray-300 rounded-md flex items-center justify-between">
                    <span className="truncate w-[85%] text-left">
                      {selectedAttrCatg.length === 0
                        ? "Select Feature Category"
                        : attrCatgList
                            .filter((item) => selectedAttrCatg.includes(item.attrCatg))
                ...
                <PopoverContent className="w-[400px] h-[300px] overflow-y-auto">
                  <div className="flex flex-col gap-2">
                    {attrCatgList.map((item) => (
                      <label key={item.attrCatg} className="flex items-center gap-2 text-md">
                        <Checkbox
                          checked={selectedAttrCatg.includes(item.attrCatg)}
                          onCheckedChange={(checked) => {
                            setSelectedAttrCatg((prev) => {
                              const updated = checked ? [...prev, item.attrCatg] : prev.filter((val) => val !== item.attrCatg);

                              setFormData((prev) => ({
                                ...prev,
                                attrApplyCatgIds: updated,
                                attrApplyCatgRequest: updated.map((catg) => ({
                                  attrId: detailData.attrId || selectedFeature.attrId,
                                  attrCatg: catg,
                                  spId: 0,
                                })),
