...
import { PORTAL_MENU_SIDEBAR } from "@/config";
import { useScrollPosition } from "@/hooks/useScrollPosition";
import { useMenus } from "@/providers";
import { ILayoutConfig, useLayout } from "@/providers";
import { deepMerge } from "@/utils";
import { PortalLayoutConfig } from ".";
...
// Initial layout provider properties, using Portal layout configuration as the default
const initalLayoutProps: IPortalLayoutProviderProps = {
  layout: PortalLayoutConfig, // Default layout configuration
...
// Create a context to manage the layout-related state and logic for Portal layout
const PortalLayoutContext =
  createContext<IPortalLayoutProviderProps>(initalLayoutProps);

// Custom hook to access the layout context in other components
const usePortalLayout = () => useContext(PortalLayoutContext);

// Provider component that sets up the layout state and context for Portal layout
const PortalLayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  // Merge the Portal layout configuration with the current layout configuration fetched via getLayout
  const layoutConfig = deepMerge(
    PortalLayoutConfig,
    getLayout(PortalLayoutConfig.name),
  );
  ...
  setMenuConfig("primary", PORTAL_MENU_SIDEBAR);
