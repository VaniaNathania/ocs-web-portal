  ...
  const OwnedUserColumn = useMemo<ColumnDef<PortalData>[]>(
    () => [
            ...
            <div className="flex items-center justify-center">
              <input
                ...
                checked={selectedOwned.some(
                  (p) =>
                    p.portalId === feature.portalId &&
                    feature.roleId == undefined,
                )}
                disabled={feature.roleId != undefined}
      ...
      },
      {
        ...
        cell({ row }) {
          return (
            <div>{row.original.roleId ? "Role Portal" : "User Portal"}</div>
          );
        },
        enableSorting: true,
        enableHiding: false,
  ...
  return (
    <div>
      <div className="flex flex-row w-full justify-between">
        <h2 className="my-auto">Portals of User{`(${ownedrows.length})`}</h2>
