...
const EditSideBar = () => {
  ...
  useEffect(() => {
    const categoryData = categorySideBar.find((cat) => String(cat.offerCatgId) === String(selectedCategoryId));
      ...
      <DialogContent className="container-fixed max-w-[768px] flex flex-col p-5 overflow-hidden">
        ...
        <DialogBody ref={parentRef}>
          <div className="flex flex-col">
            ...
            {/* {isLoading ? (
              <div className="p-4 text-gray-500">Loading category data...</div>
            ) : ( */}
            <form onSubmit={handleSubmit}>
              <div className="card-body grid gap-5">
                ...
                <div className="grid grid-cols-8 gap-2 items-center">
                  ...
                  />
                  {errors.effDate && (
                    <span className="text-red-500 text-xs mt-1 col-span-8 ml-[calc(25%+0.5rem)]">{errors.effDate}</span>
                  )}
                </div>
                {/* Actions */}
                <div className="flex justify-end gap-5">
                  <Button type="button" variant="outline" onClick={() => setShowEditSideBar(false)}>
                    Cancel
                  </Button>
