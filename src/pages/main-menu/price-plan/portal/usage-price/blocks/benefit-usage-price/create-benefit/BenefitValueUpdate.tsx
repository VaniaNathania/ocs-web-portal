...
interface BenefitValueUpdateTabProps {
  formField: any;
...
const BenefitValueUpdateTab: React.FC<BenefitValueUpdateTabProps> = ({
  formField,
  setFormField,
  errors,
  setErrors,
  ...
  const [balFlagsType, setBalFlagsType] = useState<string>(""); // "free" or "paid"
  ...
  // Function to parse balFlags and set the UI states
  const parseBalFlags = (balFlags: string | null) => {
    if (!balFlags || balFlags.length !== 8) {
      return;
    }

    // Check if it's paid (first bit is 1) or free (first bit is 0)
    const isPaid = balFlags[0] === "1";
    // Check if statistics is enabled (third bit is 1)
    const hasStatistic = balFlags[2] === "1";

    setBalFlagsType(isPaid ? "paid" : "free");
    setIsNeedStatistic(hasStatistic);
  };

  // Parse existing balFlags when formField changes (including initial load)
  useEffect(() => {
    if (formField.balFlags && formField.balFlags.length === 8) {
      parseBalFlags(formField.balFlags);
    }
  }, [formField.balFlags]);

  // Calculate balFlags based on balFlagsType and isNeedStatistic (only when user changes UI)
  useEffect(() => {
    let newBalFlags = null;

    // Only generate balFlags if user has selected a balance flags type
    if (balFlagsType === "free") {
      newBalFlags = isNeedStatistic ? "00100000" : "00000000";
    } else if (balFlagsType === "paid") {
      newBalFlags = isNeedStatistic ? "10100000" : "10000000";
    }
    // If balFlagsType is empty/null, keep newBalFlags as null

    // Always update to reflect current state (including null)
    if (newBalFlags !== formField.balFlags) {
      setFormField((prev: any) => ({
        ...prev,
        balFlags: newBalFlags,
      }));
    }
...
export default BenefitValueUpdateTab;
