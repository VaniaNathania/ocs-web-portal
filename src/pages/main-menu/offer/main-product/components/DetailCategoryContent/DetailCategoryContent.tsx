...
import RelationshipTabContent from "./RelationShipTabContent";
import PrivateOfferGroupContent from "./PrivateOfferGroupContent";
...
const CategoryDetailModal: React.FC<CategoryDetailModalProps> = ({ isOpen, onClose, category, rowData }) => {
  ...
  const { refreshCategorySidebar, isEditingMode, setIsEditingMode } = useMainProductOfferListContext();
  ...
  const { setRefreshOfferListSidebar, selectedCategoryId } = useMainProductOfferListContext();

  useEffect(() => {
    //  console.log("data move subsplan di modal", moveToSubsPlan);
  }, [moveToSubsPlan]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.status) {
        ...
        await fetchDetailCategoryContent(rowData.offerId);
  ...
  useEffect(() => {
    //  console.log("detail", detailContent);
  }, [detailContent]);
