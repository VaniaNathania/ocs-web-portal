...
const DeleteDialog = () => {
  const { showDeleteDialog, handleDeleteDialog, selectedRequestOrderCategory } =
  ...
  const doDeleteData = useCallback(async () => {
    ...
    const response = await DeleteData(
      `${API_URL}/config/request-order-category/delete/${selectedRequestOrderCategory.id}/true`,
      {
        id: selectedRequestOrderCategory.id
      }
    );
    if (response?.status) {
      setAlert((prev) => ({ ...prev, show: false, message: '' }));
      handleDeleteDialog(false, null);
      toast.success('Success Delete Request Order Category');
      reload();
      const createActivity = {
        module: 'Request Order Category',
        description: `Delete Request Order Category => ${selectedRequestOrderCategory.name}`,
        action: 'D'
      };
