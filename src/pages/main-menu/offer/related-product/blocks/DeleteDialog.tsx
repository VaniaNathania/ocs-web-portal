...
const DeleteDialog = () => {
  ...
  const { reload } = useDataGrid(); // ✅ SIMPLE: Hanya gunakan reload dari useDataGrid
  ...
  const doDeleteSideBar = useCallback(async () => {
    ...
    try {
      const deleteUrl = `${API_URL_OFFER}/offer/depend/del-depend-prod-spec/${selectedCategory?.offerId}`;
      ...
      if (response?.status) {
        ...
        //   description: `Delete Category Content => ${selectedCategory}`,
        //   action: "D",
        // };
  ...
  return (
    <Dialog open={showDeleteDialog} onOpenChange={(open) => !isDeleting && handleDeleteDialog(open, null)}>
      <DialogContent className="container-fixed max-w-md flex flex-col p-5 overflow-hidden [&>button]:hidden">
        ...
        <DialogHeader className="p-0 border-0 block">
          <Alert variant="warning">
            ...
            <span className="text-sm">You will delete the category "{selectedCategory?.offerName || "Unknown"}"</span>
          </Alert>
          {alert.show && (
        ...
        <DialogFooter className="flex justify-end items-center gap-4 mt-3">
          <Button variant="outline" onClick={() => handleDeleteDialog(false, null)} disabled={isDeleting}>
            Cancel
          </Button>
          <Button
            variant="destructive"
