...
export const AuditLogTable = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const typeLabel =
        eventType === "audit" || eventType === "audit"
          ? "Audit"
          : eventType.charAt(0).toUpperCase() + eventType.slice(1);

      let filename = `auditLog_${typeLabel}_${today}.xlsx`;
  ...
  useEffect(() => {
    ...
    if (prev) {
      ...
      prev.searching !== query?.searching;

      const isResetToPageOne = query?.page === 1 && prev.page !== 1;

      if (isFilterChange || isResetToPageOne) {
        //  console.log("🔄 Audit Log Table Reset Triggered - Key:", tableKey + 1);
  ...
  const doGetAvailableData = useCallback(
    async (page: number, limit: number, sorting: any, filter: any) => {
      try {
        ...
        // ✅ Handle searching flag - set to false setelah search pertama
        if (query?.searching) {
          updates.searching = false;
          shouldUpdate = true;
        }
