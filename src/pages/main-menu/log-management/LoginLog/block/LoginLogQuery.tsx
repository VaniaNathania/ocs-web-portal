...
import { UserMQuery } from "../hook/LogManagementProvider";
...
export const LogManagementQuery = () => {
  const { setQuery, query: currentQuery } = useLogManagement();
  ...
  useEffect(() => {
    // Hardcode Event Codes berdasarkan data yang ada di response
    const eventCodeOptions: eventCodeOption[] = [
      { text: "LOGIN_SUCCESS", value: "LOGIN_SUCCESS", codeType: "1" },
      { text: "LOGIN_FAIL", value: "LOGIN_FAIL", codeType: "1" },
      { text: "LOGOUT_SUCCESS", value: "LOGOUT_SUCCESS", codeType: "2" },
      { text: "LOGOUT_FAIL", value: "LOGOUT_FAIL", codeType: "2" },
    ];
  ...
  useEffect(() => {
    const isChanged = JSON.stringify(formQuery) !== JSON.stringify(currentQuery);
    setHasUnsavedChanges(isChanged);
  }, [formQuery, currentQuery]);

  const handleChange = <K extends keyof UserMQuery>(key: K, val: Partial<UserMQuery>[K] | null) => {
  ...
  const dynamicEventCode = () => {
    switch (formQuery.eventType) {
      case "login":
        return eventCode.filter((item) => item.codeType === "1");
      case "logout":
        return eventCode.filter((item) => item.codeType === "2");
      default:
        return eventCode;
    }
  };

  const renderDynamicEventCode = dynamicEventCode();
    ...
    <div className="bg-white m-5 rounded-md shadow-md p-5">
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="flex flex-col">
          ...
          <Select onValueChange={(val) => handleChange("eventType", val)} value={formQuery.eventType || ""}>
        ...
        <div className="flex flex-col">
          ...
          <Select onValueChange={(val) => handleChange("eventCode", val)} value={formQuery.eventCode || ""}>
            <SelectTrigger className="h-8 text-sm">
              <SelectValue placeholder="--Please Select--" />
            </SelectTrigger>
            <SelectContent>
              {renderDynamicEventCode.length > 0 ? (
                renderDynamicEventCode.map((code) => (
        ...
        <div className="flex flex-col">
          ...
          <Input type="text" placeholder="Enter Remarks" className="h-8 text-sm" value={formQuery.comments ?? ""} onChange={(e) => handleChange("comments", e.target.value)} />
        </div>
      </div>

      {/* Buttons */}
      <div className="w-full flex justify-end space-x-2 mt-4">
        <Button className="h-8 px-3 text-sm" variant="default" onClick={handleSubmit}>
          Query
        </Button>
        <Button className="h-8 px-3 text-sm" variant="outline" onClick={handleReset}>
          Reset
        </Button>
      </div>
    </div>
