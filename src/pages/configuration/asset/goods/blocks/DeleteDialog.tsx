...
const DeleteDialog = () => {
  const { showDeleteDialog, handleDeleteDialog, selectedGoods } = useGoodsContext();
  ...
  const doDeleteData = useCallback(async () => {
    ...
    const response = await DeleteData(
      `${API_URL}/config/good-category/delete/${selectedGoods.id}/true`,
      {
        id: selectedGoods.id
      }
    );
    if (response?.status) {
      ...
      toast.success('Success Delete Goods');
      reload();
      const createActivity = {
        module: 'Goods',
        description: `Delete Goods => ${selectedGoods.name}`,
        action: 'D'
      };
