...
export const AvailableRoleGrant = () => {
  const {
    lastUpdated,
    countAva,
    setCountAva,
  ...
  //   const [selectedPortals, setSelectedPortals] = useState<RoleSPID[]>([]);

  const AvailableColumn = useMemo<ColumnDef<RoleSPID>[]>(
    () => [
      {
        id: "roleId",
        enableSorting: false,
  ...
  const doGetAvailableData = useCallback(
    async (page: number, limit: number, sorting: any, columnFilter: any) => {
      try {
        const tempRole = await availableroles;
    ...
    [availableroles, search, filterBy, lastUpdated]
  );

  // Check if all visible rows are selected
  const isAllSelected = (filteredData: RoleSPID[]) => {
    return filteredData.every((row) =>
      selectedAvailable.some((selected) => selected.roleId === row.roleId)
    );
  };
  ...
  const handleSelectAll = (filteredData: RoleSPID[]) => {
    if (isAllSelected(filteredData)) {
      ...
      const merged = [
        ...selectedAvailable,
        ...filteredData.filter(
          (row) => !selectedAvailable.some((sel) => sel.roleId === row.roleId)
        ),
      ];
  ...
  const handleSelectRow = (row: RoleSPID) => {
    if (selectedAvailable.some((p) => p.roleId === row.roleId)) {
      setSelectedAvailable((prev) =>
        prev.filter((p) => p.roleId !== row.roleId)
      );
    } else {
    ...
    <div className="h-1/2 flex flex-col">
      <div className="flex flex-row w-full justify-between">
        <h2 className="my-auto">
          Ungranted Roles {`(${availableroles.length})`}
        </h2>
