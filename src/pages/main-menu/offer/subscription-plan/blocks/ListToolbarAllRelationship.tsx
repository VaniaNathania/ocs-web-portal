...
const ListToolbarAllRelationship: React.FC<ListToolbarAllRelationshipProps> = ({
  onSearchChange,
}) => {
  ...
  const [sourceOfferGroupNameOpen, setSourceOfferGroupNameOpen] =
    useState(false);
  const [targetOfferGroupNameOpen, setTargetOfferGroupNameOpen] =
    useState(false);
  const [selectedSourceOfferGroupName, setSelectedSourceOfferGroupName] =
    useState<offerGroupName | null>(null);
  const [selectedTargetOfferGroupName, setSelectedTargetOfferGroupName] =
    useState<offerGroupName | null>(null);
  const [selectedSourceOfferType, setSelectedSourceOfferType] = useState<
    string | null
  >(null);
  const [selectedTargetOfferType, setSelectedTargetOfferType] = useState<
    string | null
  >(null);
  ...
  const fetchRelationType = useCallback(async () => {
    ...
    try {
      const response = await GetData(
        `${API_URL_OFFER}/offer/common/qry-rela-type`,
        {},
      );

      if (!response?.status && response?.status !== undefined) {
        throw new Error(
          response?.message || "Failed to fetch relation type data",
        );
      }
      ...
      const uniqueList = list.reduce((acc: any[], current) => {
        const exists = acc.find(
          (item) => item.relaTypeName === current.relaTypeName,
        );
        if (!exists) {
          acc.push({
  ...
  const fetchOfferGroupName = useCallback(async () => {
    ...
    try {
      const response = await GetData(
        `${API_URL_OFFER}/offer/qry-offer-and-group-by-name-or-code`,
        {
          spId: 0,
        },
      );
  ...
  const filteredSourceOfferGroupName = useMemo(() => {
    ...
    return offerGroupName.filter(
      (offer) => String(offer.offerType) === selectedSourceOfferType,
    );
  ...
  const filteredTargetOfferGroupName = useMemo(() => {
    ...
    return offerGroupName.filter(
      (offer) => String(offer.offerType) === selectedTargetOfferType,
    );
        ...
        <div className="w-full">
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              {/* ✅ Added value prop and clear option */}
              <Select
                key={selectedTargetOfferType ?? "empty"}
                value={selectedTargetOfferType ?? undefined}
                onValueChange={handleSelectTargetOfferType}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              >
                <PopoverTrigger asChild>
                  <button
                    type="button"
                    className={`w-full flex items-center justify-between px-3 py-2 border rounded-md hover:bg-gray-50 transition-colors ${
                  ...
                  >
                    <span className="truncate">
                      {loadingSource
                        ...
                        : selectedSourceOfferGroupName
                          ? `${selectedSourceOfferGroupName.offerName || "unknown"}`
                          : "Select Source Offer Name"}
                    </span>
                      ...
                      <CommandGroup>
                        {filteredSourceOfferGroupName.map((offer, index) => (
                          <CommandItem
                            ...
                            onSelect={() => handleSelectSourceOfferName(offer)}
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {loadingTarget
                        ? "Loading Relation Types..."
                        : selectedTargetOfferGroupName
                          ? `${selectedTargetOfferGroupName.offerName || "unknown"}`
                          : "Select Target Offer Name"}
                    </span>
                    <ChevronDown className="h-4 w-4 opacity-50 shrink-0 ml-2" />
                  </button>
                </PopoverTrigger>
                      ...
                      <CommandGroup>
                        {filteredTargetOfferGroupName.map((offer, index) => (
                          <CommandItem
                            key={`${offer.offerId}-${index}`}
                            value={offer.offerName}
                            onSelect={() => handleSelectTargetOfferName(offer)}
                            className="cursor-pointer text-sm"
                          >
                            <div className="flex w-full">
                              <span className="truncate max-w-[250px]">
              ...
              >
                <PopoverTrigger asChild>
                  <button
                    type="button"
                    className={`w-full flex items-center justify-between px-3 py-2 border rounded-md hover:bg-gray-50 transition-colors ${
                      errors.relaType ? "border-red-500" : "border-gray-300"
                    } text-[13px] font-medium`}
                    disabled={loading}
                  >
                    <span className="truncate">
                      {loading
                        ...
                        : selectedRelationType
                          ? selectedRelationType.relaTypeName || "unknown"
                          : "Select Relation Type"}
                    </span>
                    <ChevronDown
                      className={`h-4 w-4 opacity-50 shrink-0 ml-2 transition-transform duration-200 ${
                        relationTypeOpen ? "rotate-180" : ""
                      }`}
                    />
                  </button>
                </PopoverTrigger>
                    ...
                    <ul className="space-y-1 max-h-48 overflow-y-auto">
                      {filteredRelationTypes.map((item) => (
                        <li key={item.relaType}>
                          <button
                            type="button"
                            className={`flex w-full text-left px-3 py-2 rounded-md text-sm hover:bg-gray-100 ${
                              selectedRelationType?.relaType === item.relaType
