...
const MappingZoneTab: React.FC<MappingZoneTabProps> = ({
  ...
  const fetchEventFeature = async (reAttrName?: string) => {
    try {
      ...
      if (reAttrName && reAttrName.trim() !== "") {
        params.append("reAttrName", reAttrName.trim());
      }

      const response = await GetData(
        `${API_URL}/mapping/re-attr/list?${params.toString()}`,
        {}
      );
  ...
  const addNewDetailItem = () => {
    ...
    if (!detailForm.mappingSrcValue) {
      toast.error("Event Feature is required");
      return;
    }
    ...
    const updatedFeatures = [...eventFeatures, newFeature];
                    ...
                    <tr key={index} className="border-t">
                      ...
                      <td className="px-4 py-3 text-sm">
                        <Button
                          ...
                          variant="outline"
                          size="icon"
                          onClick={() => removeEventFeature(feature.priority)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {detailForm.mappingSrcType === "1" && (
              <div>
                ...
                <Select
                  value={detailForm.mappingSrcValue?.toString() || ""}
                  onValueChange={(value) =>
                    setDetailForm({
                      ...detailForm,
                ...
                >
                  ...
                  <SearchSelect
                    ...
                    selectedValue={detailForm.mappingSrcValue?.toString()}
                  >
