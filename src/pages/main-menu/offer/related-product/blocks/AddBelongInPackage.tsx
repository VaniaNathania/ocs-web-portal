...
const AddBelongInPackage: React.FC<AddBelongInPackageProps> = ({
  isOpen,
  onClose = () => { },
  onSave,
  onSuccess,
  ...
  const loadExistingBelongs = useCallback(async () => {
    ...
    try {
      const response = await GetData(`${API_URL_OFFER}/offer/depend/qry-depend-prod-join-package`, {
        dependProdSpecId: currentOfferId,
      });
      ...
      const transformedExisting: BelongData[] = existingData.map((item) => ({
        dependProdSpecId: item.dependProdSpecId, // Ini adalah ID member
        offerId: item.dependProdSpecId, // Untuk konsistensi dengan interface
        offerName: item.offerName,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const added = selectedBelong.filter(
      (sel) => !initialSelectedBelongPackage.some((init) => init.dependProdSpecId === sel.dependProdSpecId)
    );
    const removed = initialSelectedBelongPackage.filter(
      (init) => !selectedBelong.some((sel) => sel.dependProdSpecId === init.dependProdSpecId)
    );
    const modified = selectedBelong.filter((sel) => {
      const initialItem = initialSelectedBelongPackage.find((init) => init.dependProdSpecId === sel.dependProdSpecId);
      ...
      if (added.length > 0) {
        payloadItems.push(
          ...added.map((item, index) => ({
            dependProdPackageId: item.dependProdSpecId, // ✅ ID package yang mau ditambahkan
            memDependProdSpecId: Number(currentOfferId), // ✅ ID package utama (72)
            seq: index + 1,
            spId: 0,
            operFlag: "A",
            defaultFlag: item.defaultFlag ?? "N",
      ...
      if (modified.length > 0) {
        payloadItems.push(
          ...modified.map((item, index) => ({
            dependProdPackageId: item.dependProdSpecId, // ✅ ID package yang mau diubah
            memDependProdSpecId: Number(currentOfferId), // ✅ ID package utama (72)
      ...
      if (removed.length > 0) {
        payloadItems.push(
          ...removed.map((item, index) => ({
            dependProdPackageId: item.dependProdSpecId, // ✅ ID package yang mau dihapus
            memDependProdSpecId: Number(currentOfferId), // ✅ ID package utama (72)
...
export default AddBelongInPackage;
