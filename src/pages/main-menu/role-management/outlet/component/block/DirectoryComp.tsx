...
export const DirectoryMenu = () => {
  const { setAvailableComponents } = useCompList();
        ...
        const baseDir: CompDir[] = [
          {
            ...
            stateDate: "string",
            privId: 0,
            privName: "Root of Directory & Menu",
          },
        ];
  ...
  const fetchChildMenus = async (
    ...
    try {
      ...
      const unique: CompDir[] = [
        ...new Map<number, CompDir>(
          res.data.map((item: CompDir) => [item.privId, item]),
        ).values(),
      ];
  ...
  const fetchCompMenus = async (parentId: number): Promise<MenuData[]> => {
    try {
      const res = await GetData(
        `${API_ROLE}/api/roles/menus/${selectedRow?.roleId}/menus/${parentId}/components`,
        {},
      );
      if (!res?.status || !res?.data) {
        throw new Error(
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <DefaultTooltip
              placement="top"
              title={data.dirName ?? data.privName}
            >
              <div
