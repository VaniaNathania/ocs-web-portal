...
interface TextCharacterFieldsProps {
  formData: any;
  onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void;
  isEditingMode: boolean;
}

const TextCharacterFields: React.FC<TextCharacterFieldsProps> = ({ formData, onChange, isEditingMode }) => {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div className="grid grid-cols-[160px_1fr] items-center gap-4">
          <span className="text-sm text-gray-800">
            {isEditingMode && <span className="text-red-500">*</span>}Text Editable:
          </span>
          {isEditingMode ? (
            ...
            <div>{formData.textEditable === "Y" ? <KeenIcon icon="check-circle" className="text-green-500 text-sm" /> : <KeenIcon icon="cross-circle" className="text-zinc-500 text-sm" />}</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-8">
        <div className="grid grid-cols-[160px_1fr] items-center gap-4">
          <span className="text-sm text-gray-800">
            {isEditingMode && <span className="text-red-500">*</span>}Error Message:
          </span>
          {isEditingMode ? (
            <input
              type="text"
...
export default TextCharacterFields;
