...
const TriggerBenefitEditDialog = ({
  ...
  const fetchTriggerBenefitDetail = async () => {
    try {
      const response = await GetData(
        `${API_URL}/trigger/benefit/balance/list`,
        {
  ...
  const onSubmit = async (data: TriggerBenefitFormType) => {
    try {
      ...
      const response = await PutData(
        `${API_URL}/trigger/benefit/balance/edit/${detailData.subBalTypeId}`,
        data,
      );
  ...
  useEffect(() => {
    if (showDialog && selectedThreshold?.tresholdId && subBalTypeId) {
      fetchTriggerBenefitDetail();
    }
  }, [showDialog, selectedThreshold?.tresholdId, subBalTypeId]);
  ...
  return (
    <Dialog open={showDialog} onOpenChange={handleClose}>
      <DialogContent className="container-fixed max-w-[1250px] flex flex-col p-4 overflow-hidden">
        ...
        <DialogBody
          className="py-0 mb-5 scrollable-y ps-0 pe-3 -me-7"
          ref={parentRef}
        >
          <div className="p-5">
