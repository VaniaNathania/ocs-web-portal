...
import { DPOfferOrderList } from "../components/modifysubscriber/model/interfaces";
import { useModSubs } from "../components/modifysubscriber/hooks/context";
import { v4 } from "uuid";
import { UUIDRec } from "@/pages/main-menu/order/models/types";
...
const ModifySubscriberDetailAddDialog = () => {
  const {
  ...
  const { ownedOffer, setOwnedOffer, uuidRec, setUuidRec } = useModSubs();
  ...
  const suggestions = availableOffer
    .filter((group) => group.offerGroupType !== "6")
    .flatMap(
      (group) =>
        group.children
          ?.filter((child) =>
...
export { ModifySubscriberDetailAddDialog };
