...
const FeatureValueModal: React.FC<FeatureValueModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        // preserve string representations so display text stays correct
        strAttrDriverList: initialData?.strAttrDriverList ?? null,
        strAttrValueLinkageList: initialData?.strAttrValueLinkageList ?? null,
      });
    }
  ...
  const showFilterText =
    ...
    formData.attrValueLinkageList.some((avl) => avl?.id.attrName)
      ? formData.attrValueLinkageList
          ...
          .join(", ")
      : formData?.strAttrValueLinkageList || "";
  ...
  const handleAddFilterFeature = useCallback((selectedFeatures: any[]) => {
    ...
    setIsAddFilterFeatureDialogOpen(false);
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      // Ensure lists are never null when saving
      const finalFormData = {
        ...formData,
        attrDriverList: formData.attrDriverList || [],
        attrValueLinkageList: formData.attrValueLinkageList || [],
      };

      if (mode === "edit") {
        // Data yang sudah ada di server (bukan temporary): hit API
        const isTemporary = !formData.attrValueId || formData.isNew || (formData.attrValueId ?? 0) > 10000;
        if (!isTemporary && formData.attrValueId) {
          ...
          // Temporary / data baru yang belum di-save ke server → langsung update state lokal
          onSave(finalFormData);
          onClose();
        }
      } else {
