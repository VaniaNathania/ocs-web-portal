...
const OfferStatusManage: React.FC<OfferStatusManageProps> = ({
  category,
  rowData,
  onClose,
}) => {
  const [offerStatus, setOfferStatus] =
    useState<OfferStatusData[]>(mockOfferStatus);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Managing offer status for:{" "}
              <span className="font-medium">{category}</span>
            </p>
            {rowData && (
              <p className="text-sm text-gray-600">
                Product Code:{" "}
                <span className="font-medium">{rowData.code}</span>
              </p>
            )}
          </div>
        </div>
