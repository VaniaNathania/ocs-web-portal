...
import { BalanceTriggerEventSchema } from "./types/forms";
import { SearchSelect } from "@/components/common/SearchSelect";

const API_URL = apiConfig.service_price_plan;
type BalanceTriggerEventFormType = z.infer<typeof BalanceTriggerEventSchema>;
...
const TriggerEventDialog = ({ showDialog, setShowDialog }: any) => {
  ...
  const methods = useForm<BalanceTriggerEventFormType>({
    resolver: zodResolver(BalanceTriggerEventSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: BalanceTriggerEventFormType) => {
    doCreateTriggerAccumulation(data);
  };

  /* actions */
  const doCreateTriggerAccumulation = useCallback(
    async (data: BalanceTriggerEventFormType) => {
      try {
        setIsLoading(true);
        const response = await PostData(
          `${API_URL}/trigger/event/balance/add`,
          data,
        );
        ...
        if (response?.status) {
          ...
          toast.success("Success Create Data Balance Event Trigger");
          resetForm();
          setShowDialog(false);
        } else {
          toast.error("Error Create Balance Event Trigger");
        }
      } catch (error) {
        toast.error("Error Create Balance Event Trigger");
      } finally {
