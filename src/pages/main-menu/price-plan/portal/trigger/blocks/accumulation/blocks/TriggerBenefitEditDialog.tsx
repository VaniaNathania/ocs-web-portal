...
import z from "zod";
import { TriggerBenefitSchema } from "../types/forms";
...
export type TriggerBenefitFormType = z.infer<typeof TriggerBenefitSchema>;

type LoadingButton = "filter" | "reset" | "export" | "refresh" | null;

const TriggerBenefitEditDialog = ({
  showDialog,
  setShowDialog,
  subBalTypeId,
}: any) => {
  const parentRef = useRef<any | null>(null);
  const { selectedThreshold, refreshBenefitList } = useTriggerCreateContext();
  ...
  const methods = useForm<TriggerBenefitFormType>({
    resolver: zodResolver(TriggerBenefitSchema),
    defaultValues: {
  ...
  >([]);
  const [selectedAcctItemOptions, setSelectedAcctItemOptions] = useState<
    {
      label: string;
      value: number;
    }[]
  ...
  const fetchAcctItemTypeDetail = async (subBalTypeId: number) => {
    try {
      const response = await GetData(
        `${API_URL}/trigger/benefit/subBalTypeLimit/list`,
        { subBalTypeId },
      );

      if (response.status) {
        const ids = response.data.map((item: any) => item.acctItemTypeId);
        const label = response.data.map((item: any) => item.acctItemTypeName);
        setValue("resultAccountItemType", ids);

        // Dapatkan full options & label-nya
        const allOptions = await fetchAcctItemType("");
        const selected = allOptions.filter((opt) => ids.includes(opt.value));
        setSelectedAcctItemOptions(selected);
      }
    } catch (error) {
      toast.error("Something went wrong while fetching account item type");
    }
  };

  const fetchTriggerBenefitDetail = async () => {
    try {
      const response = await GetData(`${API_URL}/trigger/benefit/acm/list`, {
      ...
      if (response.status) {
        const index = response.data.findIndex(
          (item: any) => item.subBalTypeId === subBalTypeId,
        );
        ...
        if (data) {
          ...
          reset({
            ...
            accountBalanceType: data.acctResId,
            cycleCeilLimit: data.ceilLimit,
            dailyCeilLimit: data.dailyCeilLimit,
            maximumDays: data.maximumDays,
            subscriberOnly: data.limitSubs,
            extendRule: data.extendRule,
            absoluteEffectiveDate: data.absEffDate,
            absoluteExpiryDate: data.absExpDate,
            offsetOfEffectiveDate: data.relEffOffset,
            effUnit: data.relEffUnit,
            dayOffset: data.dayOffset,
            durationOfAvailability: data.relExpOffset,
            expUnit: data.relExpUnit,
            relativeEffectiveTime: data.relEffTime,
            relativeExpiryTime: data.relExpTime,
            relativePeriodUnit: data.periodRelUnit,
            offsetOfAbsoluteExpiry: data.absExpOffset,
            periodType: data.periodType,
          });

          fetchAcctItemTypeDetail(data.subBalTypeId);
        }
      }
    } catch (error) {
      toast.error("Error Fetching Data. Please Check Your Connection!");
  ...
  const onSubmit = async (data: TriggerBenefitFormType) => {
    try {
      ...
      const response = await PutData(
        `${API_URL}/trigger/benefit/acm/update/${detailData.subBalTypeId}`,
        data,
      );
      ...
      if (response?.status) {
        ...
        resetForm();
        toast.success("Success Update Data Accumulation Trigger");
      } else {
        toast.error("Error Update Trigger Benefit");
      }
    } catch (error) {
      toast.error("Error Update Trigger Benefit");
    } finally {
  ...
  useEffect(() => {
    if (showDialog && selectedThreshold?.acmThresholdId && subBalTypeId) {
      fetchTriggerBenefitDetail();
    }
  }, [showDialog, selectedThreshold?.acmThresholdId, subBalTypeId]);

  useEffect(() => {
    if (showDialog && watch("effUnit") !== "W") {
      setValue("dayOffset", null);
    }
  }, [watch("effUnit"), setValue, showDialog]);

  useEffect(() => {
    if (watch("periodType") === "relative") {
      setValue("absoluteEffectiveDate", null);
      setValue("absoluteExpiryDate", null);
    } else if (watch("periodType") === "absolute") {
      ...
      setValue("effUnit", null);
      setValue("durationOfAvailability", null);
      setValue("expUnit", null);
      setValue("absoluteEffectiveDate", "");
    }
  }, [watch("periodType"), setValue]);
          ...
          <div className="p-5">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <MultiSelect<number>
                          ...
                          selectedOptions={selectedAcctItemOptions}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="flex flex-col space-y-1">
                        ...
                        {errors.relativePeriodUnit && (
                          <p className="text-xs text-red-500">
                            {errors.relativePeriodUnit.message}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
                ...
                >
                  {isLoading ? "Updating..." : "Update Trigger"}
                </Button>
              </div>
            </form>
          </div>
...
export { TriggerBenefitEditDialog };
