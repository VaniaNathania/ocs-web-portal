...
interface CreateBalanceTrigger {
  id: number;
...
const BalanceTrigger = () => {
  ...
  const {  dataPricePlan  } = usePortalData();
  ...
  //   ...initialState,
  // });
  const [formData, setFormData] = useState<CreateBalanceTrigger>(initialState);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await PostData(
        `${API_URL}/trigger/acm-trigger/create`,
        formData,
      );
  ...
  useEffect(() => {
    const fetchPricePlanDetail = async () => {
      try {
        ...
        setFormData((prev: any) => ({
          ...prev,
          offerVerId: response?.data?.offerVerList?.[0]?.offerVerId,
        }));
      } catch (error) {
        console.error(
          `Error fetching price plan detail ${dataPricePlan?.offerId} - ${dataPricePlan?.applyLevel} (${error})`,
        );
      }
    };
    ...
    const fetchAccumulationType = async () => {
      try {
        const response = await axios.get(`${API_URL}/acct/item-type-name/list`);
    ...
    const fetchTriggerMode = async () => {
      try {
        const response = await axios.get(`${API_URL}/trigger/type/list`);

        if (response && response.data) {
          setTriggerMode(response.data.data);
        }
      } catch (error) {
        toast.error("Error Fetching Data.Please Check Your Connection!");
      }
  ...
  return (
    <div className="card-header flex-wrap gap-2 border-b-0 px-5">
      <div className="flex flex-wrap gap-2 lg:gap-5 w-full">
        <form
          className="flex gap-x-5 justify-between w-full items-center"
          onSubmit={handleSubmit}
        >
          <div className="w-[90%] flex gap-3 items-center">
            <div className="w-3/12 items-baseline lg:flex-nowrap gap-5">
              <label className="form-label flex items-center gap-1 mb-2 text-[14px]">
                Effective Date
              </label>
          ...
          <div className="w-[10%] flex gap-3 items-center">
            <div className="w-6/12 items-start lg:flex-nowrap gap-5">
              <label className="form-label flex text-white items-center gap-1 mb-2 text-[14px]">
                Threshold
              </label>
            ...
            <div className="w-6/12 items-start lg:flex-nowrap gap-5">
              ...
              <div className="items-baseline flex gap-5 w-full mb-5">
                <p className="text-base text-blue-400 hover:text-blue-600 hover:cursor-pointer">
                  Detail
                </p>
                {/* <Select
