...
import { PricePlanLayoutConfig } from ".";
import {
  menuAccess,
  useRoleCheck,
...
export interface IPricePlanLayoutProviderProps {
...
const initalLayoutProps: IPricePlanLayoutProviderProps = {
  layout: PricePlanLayoutConfig, // Default layout configuration
...
const PricePlanLayoutContext =
  createContext<IPricePlanLayoutProviderProps>(initalLayoutProps);

// Custom hook to access the layout context in other components
const usePricePlanLayout = () => useContext(PricePlanLayoutContext);

// Provider component that sets up the layout state and context for PricePlan layout
const PricePlanLayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  const menuPrivAccess: menuAccess = {
    addStatus: checkMenusPriv(
      "/main-menu/price-plan/PricePlanLayoutMt",
      "addStatus",
    ),
  ...
  const layoutConfig = deepMerge(
    LayoutConfig,
    getLayout(PricePlanLayoutConfig.name),
  );
  ...
  return (
    <PricePlanLayoutContext.Provider
      value={{
...
export { PricePlanLayoutProvider, usePricePlanLayout };
