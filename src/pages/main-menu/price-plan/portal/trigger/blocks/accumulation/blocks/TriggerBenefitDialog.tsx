...
const TriggerBenefitDialog = ({ showDialog, setShowDialog }: any) => {
  ...
  const { selectedThreshold, doGetListTriggerBenefit, refreshBenefitList } =
  ...
  const methods = useForm<TriggerBenefitFormType>({
    ...
    defaultValues: {
      ...
      periodType: "absolute",
      absoluteEffectiveDate: undefined,
      absoluteExpiryDate: undefined,
  ...
  const onSubmit = async (data: TriggerBenefitFormType) => {
    if (
      data.periodType === "absolute" &&
      !data.absoluteEffectiveDate &&
      !data.absoluteExpiryDate
    ) {
      toast.error(
        "Absolute Effective Date and Absolute Expiry Date is required",
      );
      return;
    }
    try {
      ...
      const response = await PostData(
        `${API_URL}/trigger/benefit/acm/create`,
        data,
      );
  ...
  useEffect(() => {
    if (showDialog && selectedThreshold?.acmThresholdId) {
      setValue("thresholdId", selectedThreshold?.acmThresholdId);
    }
