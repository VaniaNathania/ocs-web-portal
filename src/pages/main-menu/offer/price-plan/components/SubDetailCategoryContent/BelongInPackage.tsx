...
import AddDataBelongInPackageDialog from "../../blocks/AddDataBelongInPackageDialog";
...
interface BelongInPackageProps {
  offername?: string;
  rowData: any;
...
const BelongInPackage: React.FC<BelongInPackageProps> = ({ rowData, countBelongInPackage, offerid }) => {
  ...
  const handleEditRow = (row: BelongPackageData) => {
    ...
    if (!row.offerId) {
      console.error("❌ offerId is missing from row data:", row);
    ...
    setEditingRow({
      ...
      pricePlanType: row.pricePlanType,
      defaultFlag: row.defaultFlag,
      operFlag: row.operFlag,
      seq: row.seq
    });
  };
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    if (!editingRow.offerId) {
      console.error("❌ dependProdPackageId is missing from editingRow:", editingRow);
    ...
    try {
      ...
      const payload = [{
        pricePlanPackageId: rowData.offerId || rowData.id,
        memPricePlanId: editingRow.offerId,
        spId: 0,
        operFlag: "M",
      ...
      const response = await PostData(`${API_URL_OFFER}/offer/price-plan/join-price-plan-package`, payload);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      const payload = [{
        pricePlanPackageId: selectedRowToDelete.offerId,// ✅ ID package utama
        memPricePlanId: rowData.offerId || rowData.id,  // ✅ ID package member yang dihapus
        spId: 0,
        operFlag: "D",
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const isEditing = editingRow?.offerId === row.original.offerId;
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const isEditing = editingRow?.offerId === feature.offerId;
...
export default BelongInPackage;
