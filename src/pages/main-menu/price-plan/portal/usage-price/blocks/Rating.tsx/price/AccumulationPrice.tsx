...
const AccumulationPriceComponent: React.FC<AccumulationPriceProps> = ({
  ...
  const updateItem = (index: number, field: keyof AccumulationPrice, value: any) => {
    const newData = [...data];
    ...
    if (field === "rangeEffVal" || field === "rangeExpVal") {
      ...
      if (!timeSpanUp || timeSpanUp.length === 0 || value === "" || value === "null") {
        value = null;
      } else {
            ...
            >
              ...
              <div className="col-span-2">
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    value={item.timeSpanUpId !== null ? item.timeSpanUpId.toString() : "null"}
                    ...
                    renderValue={(selected) => {
                      if (!hasTimeSpanUp) {
                        return <em style={{ color: '#999' }}>No Time Span Available</em>;
                      }
                      ...
                      if (selectedIndex >= 0 && selectedIndex < timeSpanUp.length) {
                        const timeSpanItem = timeSpanUp[selectedIndex];
                        return timeSpanItem.timeSpanName || `Time Span ${selectedIndex + 1}`;
                      }
                      return <em>Select Time Span Reference</em>;
                    }}
                    sx={{
                      ...
                      [
                        ...
                        ...timeSpanUp.map((timeSpanItem, tsIndex) => (
                          <MenuItem key={tsIndex} value={tsIndex.toString()}>
                            {timeSpanItem.timeSpanName || `Time Span ${tsIndex + 1}`}
                          </MenuItem>
                        ))
                      ]
                    )}
              ...
              <div className="col-span-1">
                <TextField
                  ...
                  onChange={(e) => updateItem(index, "calculateUnit", e.target.value)}
                ...
                <FormControl fullWidth size="small">
                  ...
                  >
                    {accumTypeList.map((accumType) => (
                      <MenuItem key={accumType.resourceId} value={accumType.resourceId.toString()}>
