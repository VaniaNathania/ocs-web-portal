...
import { DpOfferAttrRec } from "@/pages/main-menu/order/models/types";
import { Loading } from "@/pages/main-menu/role-management/block/loadingBlock";
import { useEffect, useState, useRef, Dispatch, SetStateAction } from "react";
import { DPOfferAttrList } from "../components/modifysubscriber/model/interfaces";
import {
  Select,
  SelectContent,
  SelectItem,
...
interface Props {
  ...
  setRowData: React.Dispatch<
    React.SetStateAction<{ [id: string]: AttrOrder[] }>
  >;
  rec: DPOfferAttrList[];
  setRec: Dispatch<SetStateAction<DpOfferAttrRec>>;
}

export const AttrFieldOrder = ({
  offerId,
  attrId,
  uuid,
  rowData,
  ...
  const offerAttr = rowData;
  const attr = offerAttr.find((item) => item.attrId === attrId);

  const dpOfferOrderAttr: DPOfferAttrList | undefined = rec?.find(
  ...
  if (attr?.inputType == "4") {
    return (
      <Input
        size={"sm"}
        value={dpOfferOrderAttr?.value ?? dpOfferOrderAttr?.oldValue ?? ""}
        onChange={(e) => {
          if (!dpOfferOrderAttr) {
            setRec((prev) => ({
              ...prev,
          ...
          setRec((prev) => ({
            ...prev,
            [`${offerId}#${uuid}`]: rec.flatMap((item) => {
              if (item.attrId != attrId) return item;
              return {
                ...attr,
                offerId: offerId,
                attrId: attrId,
  ...
  return (
    <Select
      value={
        dpOfferOrderAttr?.value
          ? `${dpOfferOrderAttr.value}#${dpOfferOrderAttr.valueMark}`
          : dpOfferOrderAttr?.value
            ? `${dpOfferOrderAttr?.oldValue}#${dpOfferOrderAttr?.oldValueMark}`
            : ""
      }
      onValueChange={(e) => {
    ...
    >
      ...
      <SelectContent>
        {attr?.attrValueList.map((item, index) => (
          <SelectItem key={index} value={`${item.attrValue}#${item.valueMark}`}>
            {item.valueMark}
          </SelectItem>
        ))}
      </SelectContent>
