...
import { Alert, DefaultTooltip, KeenIcon, useDataGrid } from "@/components";
import { toast } from "sonner";
import { useCallApi } from "@/hooks";
import { usePortalData } from "@/layouts/portal/price-plan/hooks/PortalDataContext";
import z from "zod";
import { TriggerAcmNotificationSchema } from "../types/forms";
...
type TriggerAcmNotificationFormType = z.infer<
  typeof TriggerAcmNotificationSchema
>;
...
const TriggerNotificationDialog = ({ showDialog, setShowDialog }: any) => {
  ...
  const methods = useForm<TriggerAcmNotificationFormType>({
    resolver: zodResolver(TriggerAcmNotificationSchema),
    defaultValues: {
    ...
    async (data: TriggerAcmNotificationFormType) => {
      try {
        const response = await PostData(
          `${API_URL}/trigger/notification/acm/create`,
          {
            ...data,
          },
        );
