...
const BalanceDeleteDialog = () => {
  const { PostData, DeleteData } = useCallApi();
  const { handleDeleteDialog, selectedDelete, showDeleteConfirm } =
  ...
  const handleCancel = () => {
    handleDeleteDialog(false, null, "balanceType");
  };
  ...
  const doDeleteAccountItem = async () => {
    try {
      const response = await DeleteData(
        `${API_URL}/account-balance/del-acct-res/${selectedDelete}`,
        {}
      );

      if (response?.status) {
        toast.success("Account Balance Type deleted successfully");
        handleDeleteDialog(false, null, "balanceType");
        reload();
      } else {
        toast.error(
          response?.message || "Failed to delete Account Balance Type"
        );
      }
    } catch (error) {
      console.error("Error deleting account Balance:", error);
      toast.error("Failed to delete Account Balance Type");
    }
  };
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-800">
                    Are you sure you want to delete this Account Balance Type?
                  </p>
                  <p className="text-sm text-red-700 mt-1">
                    This action cannot be undone and will permanently remove the
                    account Balance type from the system.
                  </p>
                </div>
              </div>
            </div>
