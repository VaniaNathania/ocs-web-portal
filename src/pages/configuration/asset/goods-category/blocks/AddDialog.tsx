...
const AddDialog = () => {
  ...
  const { showAddDialog, handleAddDialog } = useGoodsCategoryContext();
  ...
  const doCreate = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      const response = await PostData(`${API_URL}/config/good-category/create`, formField);
      if (response?.status) {
        setAlert((prev) => ({ ...prev, show: false, message: '' }));
        handleAddDialog(false);
        toast.success('Success Create Goods Category');
        reload();
        const createActivity = {
          module: 'Goods Category',
          description: `Add Goods Category => ${formField.name}`,
          action: 'C'
        };
