...
const TriggerRuleUpdate: React.FC<Props> = ({
  data,
  onChange,
  scriptToChange,
}) => {
  ...
  const { selectedAdvancedRules } = useTriggerCreateContext();
  ...
  useEffect(() => {
    if (data === null) {
      setIsActive(false);
      setFormField({ scriptTempletId: null, scriptPage: null, ruleScript: "" });
      setSelectedTemplate(null);
      setTemplateFields([]);
      setDynamicValues({});
    } else {
      setIsActive(true);
      if (JSON.stringify(data) !== JSON.stringify(formField)) {
        setFormField(data);
        const parsedJson = safeParseJsonScriptPage(data.scriptPage ?? "");
  ...
  const updateFormFieldsWithValues = (
    ...
    const scriptPage = JSON.stringify([{ "": values }]);
  ...
  // ✅ FIXED: Add missing useEffect to watch scriptTempletId changes
  useEffect(() => {
  ...
  }, [formField.scriptTempletId]); // ✅ Add dependency
    ...
    <div className="space-y-4">
      {!isActive ? (
        <div className="flex justify-between items-center">
          <h4 className="font-medium text-gray-800">Trigger Rule</h4>
          <Button
            size="sm"
      ...
      {!data && (
        <div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-md">
          No trigger rule added yet. Click "Add" to create one.
        </div>
      )}
    </div>
  );
...
export default TriggerRuleUpdate;
