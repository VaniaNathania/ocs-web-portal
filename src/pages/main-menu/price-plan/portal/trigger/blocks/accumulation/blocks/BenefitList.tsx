...
const BenefitList = () => {
  ...
  const [showTriggerBenefitEditDialog, setShowTriggerBenefitEditDialog] =
    useState(false);
  ...
  const [subBalTypeId, setSubBalTypeId] = useState<number | null>(null);

  const columns = useMemo<ColumnDef<AccumulationBenefitTriggerList>[]>(
    () => [
      {
          ...
          return (
            <div className="flex justify-center">
              <button
                ...
                onClick={() => {
                  setShowTriggerBenefitEditDialog(true);
                  setSubBalTypeId(row.original.subBalTypeId);
                }}
              >
  ...
  const confirmDelete = async (
    ...
    let ok = false;
    if (deleteType === "acmTriggerBenefit") {
  ...
  return (
    <>
      <DataGridProvider
        key={benefitListRefreshKey}
        columns={columns}
        pagination={{ size: 10 }}
        toolbar={<BenefitListToolBar />}
        layout={{ card: true }}
      ...
      >
        ...
        <TriggerBenefitEditDialog
          showDialog={showTriggerBenefitEditDialog}
          setShowDialog={setShowTriggerBenefitEditDialog}
          subBalTypeId={subBalTypeId}
        />
        <DeleteDialog
