...
const AccumulationListToolBar = () => {
  const { table, reload } = useDataGrid();
  const { GetData } = useCallApi();
  const { handleAddAccumulationDialog, selectedTrigger } =
      ...
      <div className="flex flex-wrap gap-2 lg:gap-5 w-full">
        <div className="flex justify-between w-full items-center">
          <div className="w-[80%] flex gap-3 items-center">
            <h2 className="text-lg font-semibold mb-4">Accumulation Trigger</h2>
          </div>
          <div className="flex gap-3">
            <DefaultTooltip title={"Add New Trigger"} placement={"top"}>
              <Button
                ...
                onClick={() =>
                  handleAddAccumulationDialog(true, selectedTrigger)
                }
                disabled={isLoading}
              >
                <KeenIcon icon="plus" />
...
export { AccumulationListToolBar };
