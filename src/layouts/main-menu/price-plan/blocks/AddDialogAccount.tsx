...
const AddAccountDialog = () => {
  ...
  const { showAddAccountDialog, handleAddAccountDialog } =
  ...
  const doCreateUser = async (formField: PricePlanCreateFormType) => {
    ...
    if (response?.status) {
      ...
      handleAddAccountDialog(false);
      toast.success("Success Create Priceplan ");
      reload();
  ...
  return (
    <Dialog
      ...
      onOpenChange={(open) => handleAddAccountDialog(open)}
    >
      <DialogContent className="container-fixed max-w-[1300px] flex flex-col p-0 overflow-hidden [&>button]:hidden">
        <DialogHeader className="p-6 border-b border-gray-200">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => handleAddAccountDialog(false)}
