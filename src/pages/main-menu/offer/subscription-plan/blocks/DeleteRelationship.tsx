...
const DeleteRelationship: React.FC<DeleteRelationshipProps> = ({
  offerRelaId,
  relationshipName,
  onDeleteSuccess,
  variant = "minimal",
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await DeleteData(
        `${API_URL_OFFER}/offer/rela/del-offer-rela-batch`,
        [
      ...
      const errorMessage =
        error?.message || "Unknown error occurred while deleting relationship";
      ...
      case "icon":
        return (
          <button
            ...
            disabled={disabled}
            className={`${baseClasses} text-red-500 hover:text-red-700 hover:bg-red-50 p-2 rounded-md flex items-center justify-center`}
            title={`Delete ${relationshipName}`}
          >
      ...
      case "text":
        return (
          <button
            onClick={handleDeleteClick}
            disabled={disabled}
            className={`${baseClasses} text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-1 rounded-md text-sm font-medium flex items-center gap-2`}
          >
      ...
      default:
        return (
          <button
            onClick={handleDeleteClick}
            disabled={disabled}
            className={`${baseClasses} opacity-0 group-hover/item:opacity-100 text-red-500 hover:text-red-700 text-xs px-1 py-1`}
            title={`Delete ${relationshipName}`}
          >
            ×
          </button>
          ...
          <DialogHeader className="p-0 border-0 block">
            <Alert variant="warning">
              ...
              <span className="text-sm">
                You will delete the relationship "{relationshipName}"
              </span>
            </Alert>
            {alert.show && (
