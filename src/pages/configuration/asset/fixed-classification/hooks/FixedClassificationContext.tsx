...
interface ContextProps {
  ...
  handleEditDialog: (
    show: boolean,
    selectedFixedClassification: SelectedFixedClassification | null
  ) => void;
...
interface SelectedFixedClassification {
  id: string;
  name: string;
  code: string;
  description: string;
...
const initialProps: ContextProps = {
  ...
  handleDeleteDialog: () => {},
  selectedFixedClassification: null
};

const FixedClassificationContext = createContext<ContextProps>(initialProps);

const API_URL = apiConfig.service_assets;

const FixedClassificationContextProvider = ({ children }: { children: React.ReactNode }) => {
  /* state */
  const [showEditDialog, setShowEditDialog] = useState(false);
  const [showAddDialog, setShowAddDialog] = useState(false);
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);
  const [selectedFixedClassification, setSelectedFixedClassification] =
    useState<SelectedFixedClassification | null>(null);
  const { GetData } = useCallApi();

  /* action */
  const handleEditDialog = useCallback(
    (show: boolean, selected_branch: SelectedFixedClassification | null) => {
      setSelectedFixedClassification(show ? selected_branch : null);
  ...
  const doGetListData = async (page: number, limit: number, sorting: any, filter: any) => {
    ...
    const response = await GetData(`${API_URL}/config/fixed-classification/list`, {
      limit: limit,
      page: page + 1,
      with_deleted: false,
  ...
  return (
    <FixedClassificationContext.Provider
      value={{
        showEditDialog,
        handleEditDialog,
...
export { FixedClassificationContextProvider, FixedClassificationContext };
export type { SelectedFixedClassification };
