...
interface ExpressionPriceFormProps {
  
  scriptToChange?: string;
}

const ExpressionPrice = ({scriptToChange}: ExpressionPriceFormProps) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useFormContext<DiscountPayload>();

  const currentDpRule = watch("dpRule");
  ...
  useEffect(() => {
    const hasExpressionPrice =
      currentDpRule &&
      (currentDpRule.scriptTempletId ||
        currentDpRule.ruleScript ||
        currentDpRule.jsonScriptPage ||
        currentDpRule.remarks);

    if (hasExpressionPrice) {
      setIsActive(true);
      // Parse existing jsonScriptPage to populate dynamic values
      if (currentDpRule.jsonScriptPage) {
  ...
  const updateFormFieldsWithValues = (
    ...
    setValue("dpRule", {
      ...
      jsonScriptPage,
      ruleScript,
      remarks: currentDpRule?.remarks || null,
    });
  };
  ...
  const handleDynamicValueChange = (fieldId: string, value: string) => {
    ...
    if (selectedTemplate && currentDpRule?.scriptTempletId != null) {
  ...
  const compileRuleScript = async () => {
    try {
      const response = await PythonData(
        ...
        {
          phytonScript: currentDpRule?.ruleScript,
        }
      );
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 items-start">
                  <div className="flex-1 min-w-0">
                    <Select
                      value={
                        currentDpRule?.scriptTempletId?.toString() ||
                        ""
                      }
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    ...
                    <Button
                      ...
                      disabled={!currentDpRule?.ruleScript?.trim()}
                    >
                      Run
                    </Button>
                  </div>
                  <Textarea
                    value={currentDpRule?.ruleScript || ""}
                    onChange={(e) => handleRuleScriptChange(e.target.value)}
                    className="text-xs font-mono"
                    rows={12}
                    placeholder="Enter your rule script here..."
                    disabled={!!currentDpRule?.scriptTempletId}
                  />
                </div>
                <div className="space-y-1">
                  <Label className="text-sm font-medium">
                  ...
                  <Textarea
                    value={currentDpRule?.jsonScriptPage || ""}
                    readOnly
