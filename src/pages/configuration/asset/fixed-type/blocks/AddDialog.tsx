...
const AddDialog = () => {
  ...
  const { showAddDialog, handleAddDialog } = useFixedTypeContext();
  const { GetData } = useCallApi();
  const { reload } = useDataGrid();
  const { PostData } = useCallApi();

  const [fixedClassification, setFixedClassification] = useState<FixedClassificationList[]>([]);
  ...
  const doCreate = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      const response = await PostData(`${API_URL}/config/fixed-type/create`, formField);
      if (response?.status) {
        setAlert((prev) => ({ ...prev, show: false, message: '' }));
        handleAddDialog(false);
        toast.success('Success Create Fixed Type');
        reload();
        const createActivity = {
          module: 'Fixed Type',
          description: `Add Fixed Type => ${formField.name}`,
          action: 'C'
        };
        ...
        <DialogHeader className="p-0 border-0">
          ...
          <div className="flex items-center justify-between flex-wrap grow">
            <div className="flex flex-col justify-center gap-2">
              <h1 className="text-xl font-semibold leading-none text-gray-900">Add Fixed Type</h1>
                      ...
                      <div className="grow">
                        <Select
                          ...
                          onValueChange={(fixedClassificationId) =>
                            setFormField((prev) => ({ ...prev, fixedClassificationId }))
                          }
                        >
                          <SelectTrigger>
                          ...
                          <SelectContent>
                            {fixedClassification.map((fixedClassification) => (
                              <SelectItem
                                key={fixedClassification.id}
                                value={fixedClassification.id}
                              >
                                {fixedClassification.name}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
