...
export const AvailableUser = () => {
  ...
  //   const [selectedPortals, setSelectedPortals] = useState<UserData[]>([]);

  const AvailableColumn = useMemo<ColumnDef<UserData>[]>(
    () => [
      {
        id: "userId",
        enableSorting: false,
      ...
      },
      {
        ...
        id: "address",
        header: ({ column }) => (
          <DataGridColumnHeader className="" title="Address" column={column} />
        ),
        enableSorting: true,
        enableHiding: false,
      },
  ...
  const doGetAvailableData = useCallback(
    async (page: number, limit: number, sorting: any, filter: any) => {
      ...
      const response = await GetData(
        `${API_ROLE}/api/roles/${selectedRow?.roleId}/users/ungrant/filter`,
        params,
      );
  ...
  const isAllSelected = (filteredData: UserData[]) => {
    return filteredData.every((row) =>
      selectedAvailable.some((selected) => selected.userId === row.userId),
    );
  };
  ...
  const handleSelectAll = (filteredData: UserData[]) => {
    if (isAllSelected(filteredData)) {
      ...
      const merged = [
        ...selectedAvailable,
        ...filteredData.filter(
          (row) => !selectedAvailable.some((sel) => sel.userId === row.userId),
        ),
      ];
  ...
  const handleSelectRow = (row: UserData) => {
    if (selectedAvailable.some((p) => p.userId === row.userId)) {
      setSelectedAvailable((prev) =>
        prev.filter((p) => p.userId !== row.userId),
      );
    } else {
  ...
  return (
    <div>
      <div className="flex flex-row w-full justify-between">
        <h2 className="my-auto">Ungranted User {`(${countAva})`}</h2>
