...
export const SystemLogContext = createContext<SystemLogContextType>({
  user: [],
  setUser: () => {},
...
export const SystemLogProvider = ({
  children,
}: {
  ...
  const fetchUser = async (data: Partial<UserMQuery> = {}) => {
    ...
    const payload = {
      ...
      sortBy: mergedQuery.sortBy || "LOG_ID",
      sortDirection: mergedQuery.sortDirection || "desc",
      eventType: mergedQuery.eventType || null,
      eventCode: mergedQuery.eventCode || null,
      userId: user?.user.id,

      // eventSrc: mergedQuery.eventSrc || null,
      // bisTransId: mergedQuery.bisTransID || null,
      comments: mergedQuery.comments || null,
      // logid: mergedQuery.logId || null,
      // srcIp: mergedQuery.srcIp || null,
    ...
    try {
      const url = `${API_LOG}/api/log-management/sytem-log-list`;
