...
interface MultiSelectProps {
  ...
  options?: AcctItemTypeOption[];
  placeholder?: string;
  searchPlaceholder?: string;
  maxHeight?: string;
  value: AcctItemTypeOption[];
  onChange: (items: AcctItemTypeOption[]) => void;
}
...
const MultiSelect = ({
  ...
  const groupedOptions = filteredOptions.reduce(
    ...
    {} as Record<string, AcctItemTypeOption[]>
  );

  const handleSelectItem = (option: AcctItemTypeOption) => {
                  ...
                  {value.length === 0 ? (
                    ...
                    <div className="p-2">
                      {value.map((item) => (
                        <div
                          key={item.id}
                          className="flex items-center justify-between p-3 mb-2 transition-shadow bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md"
                        >
                          <div>
