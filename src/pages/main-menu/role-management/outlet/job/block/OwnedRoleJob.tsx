...
export const OwnedRoleJob = () => {
  ...
  const OwnedUserColumn = useMemo<ColumnDef<JobData>[]>(
    () => [
      {
        id: "select",
        enableSorting: false,
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center justify-center">
              <input
                type="checkbox"
                checked={selectedOwned.some((p) => p.jobId === feature.jobId)}
  ...
  const doGetOwnedRoleData = useCallback(
    async (page: number, limit: number, sorting: any, filter: any) => {
      ...
      const response = await GetData(
        `${API_ROLE}/api/stafforg/jobs/${selectedRow?.roleId}/grant/filter`,
        params,
      );
  ...
  const handleSelectRow = (row: JobData) => {
    if (selectedOwned.some((p) => p.jobId === row.jobId)) {
      setSelectedOwned((prev) => prev.filter((p) => p.jobId !== row.jobId));
    } else {
  ...
  return (
    <div>
      <div className="flex flex-row w-full justify-between">
        <h2 className="my-auto">Jobs of the Roles{`(${countOwned})`}</h2>
      ...
      <DataGridProvider
        key={`available-features-grid-${search}-${filterBy}-${selectedRow?.roleId}-${lastUpdated}`}
