...
import z from "zod";
import { TriggerAcmEventSchema } from "../types/forms";
import { Controller, useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { SearchSelect } from "@/components/common/SearchSelect";

const API_URL = apiConfig.service_price_plan;
type TriggerAcmEventFormType = z.infer<typeof TriggerAcmEventSchema>;

const TriggerEventDialog = ({ showDialog, setShowDialog }: any) => {
  const parentRef = useRef<any | null>(null);
  const { selectedThreshold, refreshEventList } = useTriggerCreateContext();
  const { PostData, PutData, GetData } = useCallApi();

  const methods = useForm<TriggerAcmEventFormType>({
    resolver: zodResolver(TriggerAcmEventSchema),
    defaultValues: {
      acmThresholdId: selectedThreshold?.acmThresholdId,
  ...
  const onSubmit = async (data: TriggerAcmEventFormType) => {
    doCreateTriggerAccumulation(data);
  };

  /* actions */
  const doCreateTriggerAccumulation = useCallback(
    async (data: TriggerAcmEventFormType) => {
      try {
        setIsLoading(true);
        const response = await PostData(
          `${API_URL}/trigger/event/acm/add`,
          data,
        );
        ...
        if (response?.status) {
          ...
          toast.success("Success Create Data Accumulation Event Trigger");
          resetForm();
          setShowDialog(false);
        } else {
          toast.error("Error Create Accumulation Event Trigger");
        }
      } catch (error) {
        toast.error("Error Create Accumulation Event Trigger");
      } finally {
