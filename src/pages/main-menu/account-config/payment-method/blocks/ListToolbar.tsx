...
const ListToolbar = () => {
  ...
  const { handleShowDialog, showDialog } = usePaymentMethod();
  const [searchValue, setSearchValue] = useState<string>(
    (table.getColumn("paymentMethodName")?.getFilterValue() as string) ?? "",
  );

  useEffect(() => {
    const timer = setTimeout(() => {
      table.getColumn("paymentMethodName")?.setFilterValue(searchValue);
