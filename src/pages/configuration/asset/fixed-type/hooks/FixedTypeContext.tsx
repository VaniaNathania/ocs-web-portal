...
interface FixedClassificationListProps {
  code: string;
  created_at: string;
  deleted_at: string;
  id: string;
...
interface ContextProps {
  ...
  handleEditDialog: (show: boolean, selectedFixedType: SelectedFixedType | null) => void;
  showAddDialog: boolean;
  handleAddDialog: (show: boolean) => void;
  showDeleteDialog: boolean;
  handleDeleteDialog: (show: boolean, selectedFixedType: SelectedFixedType | null) => void;
  selectedFixedType: SelectedFixedType | null;
  fixedClassificationList: FixedClassificationListProps[];
}

interface SelectedFixedType {
  id: string;
  code: string;
  name: string;
  description: string;
...
const initialProps: ContextProps = {
  ...
  handleDeleteDialog: () => {},
  selectedFixedType: null,
  fixedClassificationList: []
};

const FixedTypeContext = createContext<ContextProps>(initialProps);

const API_URL = apiConfig.service_assets;

const FixedTypeContextProvider = ({ children }: { children: React.ReactNode }) => {
  /* state */
  const [showEditDialog, setShowEditDialog] = useState(false);
  const [showAddDialog, setShowAddDialog] = useState(false);
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);
  const [selectedFixedType, setSelectedFixedType] = useState<SelectedFixedType | null>(null);
  const [fixedClassificationList, setFixedClassificationList] = useState<
    FixedClassificationListProps[]
  >([]);
  const { GetData } = useCallApi();

  /* action */
  const handleEditDialog = useCallback(
    (show: boolean, selected_branch: SelectedFixedType | null) => {
      setSelectedFixedType(show ? selected_branch : null);
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      {
        accessorFn: (row) => row.fixed_classification.name,
        id: 'fixed_classification.name',
        header: ({ column }) => (
          <DataGridColumnHeader title="Fixed Classification" column={column} />
        ),
        enableSorting: false,
        enableHiding: false,
        meta: {
...
export { FixedTypeContextProvider, FixedTypeContext };
export type { SelectedFixedType };
