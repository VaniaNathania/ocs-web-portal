...
const DeleteDialog = () => {
  const { showDeleteDialog, handleDeleteDialog, selectedFixedClassification } =
  ...
  const doDeleteData = useCallback(async () => {
    ...
    const response = await DeleteData(
      `${API_URL}/config/fixed-classification/delete/${selectedFixedClassification.id}/true`,
      {
        id: selectedFixedClassification.id
      }
    );
    if (response?.status) {
      setAlert((prev) => ({ ...prev, show: false, message: '' }));
      handleDeleteDialog(false, null);
      toast.success('Success Delete Fixed Classification');
      reload();
      const createActivity = {
        module: 'Fixed Classification',
        description: `Delete Fixed Classification => ${selectedFixedClassification.name}`,
        action: 'D'
      };
