...
import { RecurringCreateRatingFormType } from "../AddPriceDialog";
...
const ExpressionPrice = () => {
  ...
  const { setValue, watch } = useFormContext<RecurringCreateRatingFormType>();
  ...
  useEffect(() => {
    const hasExpressionPrice =
      ...
      (currentExpressionPrice.scriptTempletId ||
        ...
        currentExpressionPrice.advancedBenefitRemarks);

    if (hasExpressionPrice) {
      setIsActive(true);
      // Parse existing jsonScriptPage to populate dynamic values
      if (currentExpressionPrice.jsonScriptPage) {
        const parsedJson = safeParseJsonScriptPage(currentExpressionPrice.jsonScriptPage);
  ...
  const fetchScriptDetail = async (
    ...
    try {
      ...
      const xmlToParse = isTemplateChange
        ...
        : currentExpressionPrice?.ruleScript && currentExpressionPrice.ruleScript.trim() !== ""
  ...
  const updateFormFieldsWithValues = (
    ...
    setValue("expressionPrice", {
      ...
      jsonScriptPage,
      ruleScript,
      advancedBenefitRemarks: currentExpressionPrice?.advancedBenefitRemarks || null,
    });
  };
  ...
  const handleRemarksChange = (value: string) => {
    setValue("expressionPrice", {
      ...currentExpressionPrice,
      advancedBenefitRemarks: value || null,
    });
  };
  ...
  useEffect(() => {
    if (currentExpressionPrice?.scriptTempletId) {
      ...
      fetchScriptDetail(currentExpressionPrice.scriptTempletId, isTemplateChange);
    }
  }, [currentExpressionPrice?.scriptTempletId]);
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 items-start">
                  <div className="flex-1 min-w-0">
                    <Select
                      value={currentExpressionPrice?.scriptTempletId?.toString() || ""}
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="text"
                  autoComplete="off"
                  value={currentExpressionPrice?.advancedBenefitRemarks || ""}
